package ru.kotopushka.j2c.translator.processor.cpp.impl.array;
    ...
    public void translate(MethodContext context, AbstractInsnNode insnNode, MethodNode method) {
        ...
        if (insnNode instanceof TypeInsnNode type) {
            if (insnNode.getOpcode() == ANEWARRAY) {
                context.output().pushMethodLine("if (cstack%s.i < 0) throw_re(env, %s, %s, 404); else { cstack%s.l = env->NewObjectArray(cstack%s.i, %s, nullptr); }"
                        .formatted(
                                context.getStackPointer().peek() - 1,
                                context.getConstantPool().pushUtf("java/lang/NegativeArraySizeException"),
                                context.getConstantPool().pushUtf("ARRAYLENGTH negative"),
                                context.getStackPointer().peek() - 1,
                                context.getStackPointer().peek() - 1,

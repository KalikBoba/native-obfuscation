package ru.kotopushka.j2c.translator.processor.cpp.impl.constant;
...
public class ConstProcessor extends BaseProcessor {
    ...
    public void translate(MethodContext context, AbstractInsnNode insn, MethodNode method) {
        switch (insn.getOpcode()) {
            ...
            case FCONST_0 -> context.output().pushMethodLine("cstack%s.f = 0.0f;".formatted(context.getStackPointer().peek()));
            case FCONST_1 -> context.output().pushMethodLine("cstack%s.f = 1.0f;".formatted(context.getStackPointer().peek()));
            case FCONST_2 -> context.output().pushMethodLine("cstack%s.f = 2.0f;".formatted(context.getStackPointer().peek()));

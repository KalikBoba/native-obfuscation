...
public class MethodProcessor {
    ...
    static {
        processors.addAll(ReflectionUtils.getClasses("ru.kotopushka.j2c.translator.processor.cpp.impl", BaseProcessor.class));
        ...
        Compiler.COMPILE_LOGGER.info("Loaded {} processors. [{}/{}] ({} instructions supported)",
        ...
        if (!unsupportedInsns.isEmpty()) {
            ...
            Compiler.COMPILE_LOGGER.warn("Unsupported instructions: {}", unsupportedInsnNames);
        }
    }
        ...
        for (AbstractInsnNode abstractInsnNode : method.instructions) {
            if (abstractInsnNode instanceof MethodInsnNode insnNode) {
                if (insnNode.getOpcode() == Opcodes.INVOKESTATIC) {
                    ReferenceNode referenceNode = context.output().allocateOrGetMethodNode(context,
                            insnNode.owner,
                            insnNode.name,
                            insnNode.desc,
                            true,
                    ...
                    context.output().getCurrentWriter().getReferences().add(referenceNode);
                }
            } else if (abstractInsnNode instanceof FieldInsnNode insnNode) {
                if (insnNode.getOpcode() == Opcodes.GETSTATIC || insnNode.getOpcode() == Opcodes.PUTSTATIC) {
                    ReferenceNode referenceNode = context.output().allocateOrGetFieldNode(context,
                            insnNode.owner,
                            insnNode.name,
                            insnNode.desc,
                            true,
                    ...
                    context.output().getCurrentWriter().getReferences().add(referenceNode);
//                    System.out.println(((FieldInsnNode) abstractInsnNode).name + ((FieldInsnNode) abstractInsnNode).desc);
                }
            } else if (abstractInsnNode instanceof LdcInsnNode ldcInsnNode) {
                if (ldcInsnNode.cst instanceof String) {
                    context.output().getCurrentWriter().getReferences().add(context.output().pushJavaString(context.getConstantPool().pushUtf((ldcInsnNode.cst.toString()))));
                }
            }
        }
...
//        if (!isClinit(method))
//            context.output().beginV2(method);
        if (isClinit(method)) {
            context.output().pushMethodLine("""
            std::string hwid = get_random_string(16);
                        writers[%s] = *new Writer(hwid, get_random_string(32));
            
            """.formatted(context.getCurrentWriterIndex()));
            linker.end();
            context.output().pushMethodLine(linker.getMethods().toString());
            context.output().pushString(context.output().getClassReferences());
//            context.output().pushString(context.output().getMethodReferences());
//            context.output().pushString(context.output().getFieldReferences());
//            context.output().pushString("writers[%s].push();\n".formatted(context.getCurrentWriterIndex()));
//            context.output().pushString("""
//
//                    if(((__int64)classes[0] == ((writers[0].address_at(0) ^ writers[0].private_key()[1] ^ writers[0].hwid()[5])))) {
...
//                    """);
        ...
        for (TryCatchBlockNode tryCatchBlock : method.tryCatchBlocks) {
            ...
            catchHandlers.computeIfAbsent(catchLabel, a -> new ArrayList<>())
...
//            context.output().endV2(method);
    ...
    private void initializeLocals(MethodNode method, List<Type> argTypes, StringBuilder stackBuilder, StringBuilder localBuilder) {
        ...
        IntStream.range(1, method.maxLocals).forEach(h ->
    ...
    private void writeLocals(MethodContext writer, List<Type> argTypes, List<String> argNames) {
        ...
        for (int i = 0; i < argTypes.size(); ++i) {
            ...
            writer.output().pushMethodLine("clocal%s%s = ".formatted(localIndex, stackType));
    ...
    private void processInstructions(MethodNode method, MethodContext writer, Map<LabelNode, String> catchLabels) {
        if (TranslatorConfiguration.IMP.MAIN.DEBUG_ENABLED) Compiler.COMPILE_LOGGER.info("Processing method {}{}", method.name, method.desc);
        ...
        method.instructions.forEach(insn -> {
            if (insn instanceof LabelNode labelNode) {
                ...
                if (TranslatorConfiguration.IMP.MAIN.DEBUG_ENABLED)
                    Compiler.LOGGER.info("Label start (name={})", labelName);
                return;
            }
            ...
            if (insn.getOpcode() != -1) {
                ...
                writer.output().pushLine().pushTab().pushString("    { // %s; Stack pointer: %d".formatted(opcodeName, writer.getStackPointer().peek()));
            }
            ...
            processors.stream()
                    ...
                    .ifPresentOrElse(
                            compiler -> {
//                                writer.output().pushMethodLine("env->ExceptionDescribe();");
                                ...
                                if (TranslatorConfiguration.IMP.MAIN.DEBUG_ENABLED)
                                    Compiler.COMPILE_LOGGER.info("Updated pointer from {} to {} (diff: {}) after instruction {} [{}]",
                                            currentPointer, updatedPointer, diff, BaseUtils.getOpcodeString(insn.getOpcode()), insn.getOpcode());
                                InstructionRecordHandler.setLastOpcode(insn);
                            },
                            () -> Compiler.COMPILE_LOGGER.error("Unsupported insn {} [{}]",
                                    BaseUtils.getOpcodeString(insn.getOpcode()),
                                    insn.getOpcode())
                    );
            if (insn.getOpcode() > 0) writer.output().pushLine().pushTab().pushString("    }\n");
...
//                writer.output().pushMethodLine("env->ExceptionDescribe(); std::cout << %s << std::endl;".formatted(indx++));
//                writer.output().pushMethodLine("if (env->ExceptionCheck()) { env->ExceptionDescribe(); cstack_exception.l = env->ExceptionOccurred(); env->ExceptionClear();/* goto L_EXCEPTION;*/ }");
            }
        });
    }
    ...
    private void writeCatchHandlers(MethodContext context, MethodNode method, Map<String, List<CatchInfo>> catchHandlers) {
        ...
        if(method.exceptions != null && !method.exceptions.isEmpty()) {
            method.exceptions.forEach(System.out::println);
        }
